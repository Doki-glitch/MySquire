...
const habilidadeSchema = z.object({
    ...
    descricao: z.string(
        { message: "A descrição deve possuir só caractere string e no mínimo um caractere" }).min(1),                        
})
...
router.post("/", async (req, res) => {
    ...
    const { nome, valor, profissaoId, descricao } = valida.data

    try {
      const habilidade = await prisma.habilidade.create({
        data: { nome, valor, profissaoId, descricao }
      })
